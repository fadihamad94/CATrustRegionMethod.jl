...
function recordFactorizationStats!(
    ...
    stats::TrustRegionSubproblemSolvers.FactorizationStats,
)
    return record_factorization_stats!(algorithm_counter, stats)
end

function recordIterativeStats!(
    algorithm_counter::AlgorithmCounter,
    stats::TrustRegionSubproblemSolvers.IterativeStats,
)
    return record_iterative_stats!(algorithm_counter, stats)
end

function recordSubproblemStats!(
    algorithm_counter::AlgorithmCounter,
    result::TrustRegionSubproblemSolvers.TrustRegionSubproblemResult,
)
    recordFactorizationStats!(algorithm_counter, result.factorizations)
    recordIterativeStats!(algorithm_counter, result.iterative_stats)
    return algorithm_counter
end

"""Compatibility wrapper that delegates the subproblem solve to the sibling package."""
function solveTrustRegionSubproblem(
    ...
    sparse_cholesky_workspace::OptionalSparseCholeskyWorkspace = SparseCholeskyWorkspace(),
    handle_hard_case::Bool = DEFAULT_HANDLE_HARD_CASE,
)
    result = TrustRegionSubproblemSolvers.solveTrustRegionSubproblem(
        problem_name,
        g,
        H,
        δ,
    ...
    recordSubproblemStats!(algorithm_counter, result)
    return result.success, result.delta, result.direction, result.hard_case
end

"""
  sub_routine_trust_region_sub_problem_solver(fval_current, gval_current, hessian_current, x_k, δ_k, γ_1, γ_2, r_k, min_grad_norm, nlp, print_level, trust_region_subproblem_solver)
  The method computes a search direction that solves the trust-region subproblem.

  See the TrustRegionSubproblemSolvers package for implementation details,
  input descriptions, and output descriptions.
"""
function sub_routine_trust_region_sub_problem_solver(
    ...
    γ_3::Float64,
    r_k::Float64,
    min_grad_norm::Float64,
    problem_name::String,
    algorithm_counter::AlgorithmCounter,
    print_level::Int64,
    trust_region_subproblem_solver::String = DEFAULT_TRUST_REGION_SUBPROBLEM_SOLVER,
    use_backup_trust_region_subproblem_solver::Bool =
        DEFAULT_USE_BACKUP_TRUST_REGION_SUBPROBLEM_SOLVER,
    sparse_cholesky_workspace::OptionalSparseCholeskyWorkspace = SparseCholeskyWorkspace(),
    handle_hard_case::Bool = DEFAULT_HANDLE_HARD_CASE,
    ...
    if trust_region_subproblem_solver == "OLD"
        # Note: we intentionally do not count factorizations for this method
        # because SVD dominates the cost.
    ...
    elseif trust_region_subproblem_solver == DIRECT_NEW_SOLVER
        ...
        throw(
            ArgumentError(
                "Unknown trust-region subproblem solver " *
                "$(repr(trust_region_subproblem_solver)).",
            ),
        )
    end
    end_time_temp = time()
...
"""
    CAT_solve(model[, termination_criteria, algorithm_params])

Solve an unconstrained JuMP or NLPModels model. The returned seven-tuple is
`(x, status, iteration_stats, algorithm_counter, iterations, execution_time,
best_gradient_x)`. The first six fields preserve the original API;
`best_gradient_x` is the evaluated point paired with the
`min_gradnorm_fval`/`min_gradnorm` history columns.
"""
...
function CAT_solve(
    ...
    if NLPModels.get_ncon(nlp_raw) > 0
...
"""
  ...
  # Inputs:
	...
	- `δ::Float64`. The initial warm start value for δ.
  ...
  # Outputs:
	...
	- `best_gradient_x::Vector{Float64}`. The evaluated point with the smallest
	  gradient norm encountered by the algorithm.
"""
function optimize(
    ...
    dense_hessian_threshold = algorithm_params.dense_hessian_threshold
    ...
    try
        ...
        min_grad_norm = norm(gval_current)
        min_gnorm_obj = deepcopy(fval_current)

        compute_hessian = false
        if min_grad_norm <= gradient_termination_tolerance
            if print_level >= 0
                println("*********************************Iteration Count: ", 1)
            end
            push!(iteration_stats, (1, fval_current, min_grad_norm, fval_current, min_grad_norm))
            end_time_ = time()
        ...
        if r_k <= 0.0
            matrix_l2_norm_val = matrix_l2_norm(
                hessian_current,
                num_iter = DEFAULT_POWER_ITERATION_MAX_ITERATIONS,
            )
        ...
        while k <= MAX_ITERATIONS
            ...
            if !success_subproblem_solve
                ...
                total_execution_time = end_time_ - start_time_
                return x_k,
                TerminationStatusCode.TRUST_REGION_SUBPROBLEM_ERROR,
                iteration_stats,
                algorithm_counter,
                k,
                total_execution_time,
                ...
                if fval_next <=
                   ...
                   (1 + abs(fval_current)) * eval_offset
                    ...
                    if isnan(temp_norm)
                        ...
                        if temp_norm < min_grad_norm
                            min_grad_norm = temp_norm
                            min_gnorm_obj = fval_next
                            best_gradient_x = copy(x_k + d_k)
                        end
                    end
                end
            ...
            if ρ_k >= 0.0 && (fval_next <= fval_current)
                ...
                start_time_temp = time()
                gval_next = temp_grad
                if isnan(min_grad_norm)
                    if print_level >= 0
                        println("$k. min_grad_norm is NaN")
                    end
                    @warn "$k min_grad_norm is NaN."
                end
                end_time_temp = time()
                total_time_temp = end_time_temp - start_time_temp
                if print_level >= 2
                ...
                # The rejected trial leaves the iterate and its derivatives unchanged.
                compute_hessian = false
            end
            if print_level >= 1
            ...
            push!(iteration_stats, (k, fval_current, norm(gval_current), min_gnorm_obj, min_grad_norm))
        ...
        if e isa OverflowError || e isa DomainError || e isa PosDefException
            status = TerminationStatusCode.NUMERICAL_ERROR
        end
        if isa(e, TrustRegionSubproblemError)
            ...
            TrustRegionSubproblemSolvers.printFailures(
                problem_name,
                failure_reason_6a,
                failure_reason_6b,
                failure_reason_6c,
...
function evalFunction(
    ...
    return evaluate_function(nlp, x, algorithm_counter)
end
...
function evalGradient(
    ...
    return evaluate_gradient(nlp, x, algorithm_counter)
end
...
function evalHessian(
    ...
    constraint_count =
        nlp isa MathOptInterface.NLPBlockData ? length(nlp.constraint_bounds) : 0
    return evaluate_hessian(
        nlp,
        x,
        algorithm_counter;
        constraint_count,
        normalize_moi = restoreFullMatrix,
    )
end

...
function test_optimize_second_order_model_δ_0_H_positive_semidefinite_starting_on_global_minimizer()
    ...
    g = grad(nlp, x_k)
    H = hess(nlp, x_k)
    status, δ_k, d_k = CAT.optimizeSecondOrderModelOldApproach(g, H, δ, γ_2, r)
    γ_1 = 1e-2
...
function test_phi_negative_one()
    ...
    ϵ = 0.8
    r = 0.2
    Φ_δ = CAT.phiOldApproach(g, H, δ, ϵ, r)
    @test Φ_δ == -1
end
...
function test_find_interval_with_both_phi_zero_starting_from_phi_zero()
    ...
    ϵ = 0.8
    r = 0.2
    δ, δ_prime = CAT.findintervalOldApproach(g, H, δ, ϵ, r)
    @test δ == δ_prime == 64.0
    Φ_δ = CAT.phiOldApproach(g, H, δ, ϵ, r)
    @test Φ_δ == 0

    Φ_δ_prime = CAT.phiOldApproach(g, H, δ_prime, ϵ, r)
    @test Φ_δ_prime == 0
...
function test_find_interval_with_phi_δ_positive_one_phi_δ_prime_negative_one()
    ...
    Φ_δ_prime = CAT.phiOldApproach(g, H, δ_prime, γ_2, r)
    @test Φ_δ_prime == 0
...
function test_bisection_with_starting_on_root_δ_zero()
    ...
    Φ_δ_m = CAT.phiOldApproach(g, H, δ_m, γ_2, r)
    @test Φ_δ_prime == 0

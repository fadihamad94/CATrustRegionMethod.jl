...
function MOI.supports(
    ::CATSolver, ::MOI.ObjectiveFunction{MOI.VariableIndex}
)
    return true
end

function MOI.supports(
    ::CATSolver, ::MOI.ObjectiveFunction{MOI.ScalarAffineFunction{Float64}}
)
    return true
end

function MOI.supports(
    ::CATSolver, ::MOI.ObjectiveFunction{MOI.ScalarQuadraticFunction{Float64}}
)
    return true
end
...
function MOI.set(
    model::CATSolver, ::MOI.ObjectiveSense, sense::MOI.OptimizationSense
)
    model.sense = sense
    return
end
...
function MOI.supports(
    ::CATSolver, ::MOI.VariablePrimalStart, ::Type{MOI.VariableIndex}
)
    return true
end
...
function MOI.get(
    model::CATSolver, attr::MOI.VariablePrimal, vi::MOI.VariableIndex,
)

...
function CAT_solve(m::JuMP.Model, pars::Class_parameters)
    nlp_raw = MathOptNLPModel(m)
    return CAT_solve(nlp_raw, pars::Class_parameters)
end
...
function CAT_solve(solver::CATSolver, pars::Class_parameters)
	...
	δ = 0.0
	return CAT_v2(solver.nlp_data, pars, x, δ, subproblem_solver_methods.OPTIMIZATION_METHOD_DEFAULT)
end

function CAT_solve(nlp_raw::NLPModels.AbstractNLPModel, pars::Class_parameters)
	if ncon(nlp) > 0
		throw(ErrorException("Constrained minimization problems are unsupported"))
	end
	termination_conditions_struct = pars.termination_conditions
	initial_radius_struct = pars.initial_radius
	β_1 = pars.β_1
	θ = pars.θ
	ω_1 = pars.ω_1
	ω_2 = pars.ω_2
	γ_1 = pars.γ_1
	γ_2 = pars.γ_2
	print_level = pars.print_level
	compute_ρ_hat_approach = pars.compute_ρ_hat_approach
	radius_update_rule_approach = pars.radius_update_rule_approach

	problem = Problem_Data(nlp_raw, termination_conditions_struct, initial_radius_struct, β_1, θ, ω_1, ω_2,
							γ_1, γ_2, print_level, compute_ρ_hat_approach, radius_update_rule_approach)

	subproblem_solver_method = subproblem_solver_methods.OPTIMIZATION_METHOD_DEFAULT
	δ = 0.0
	x = nlp_raw.meta.x0
	return CAT(problem, x, δ, subproblem_solver_method)
end
...
function evalHessian(nlp, x)
	if typeof(nlp) == AbstractNLPModel || typeof(nlp) ==  MathOptNLPModel
		...
		# hessian = Symmetric(spzeros(Float64, (length(x), length(x))))
...
function CAT_v2(nlp, problem::Class_parameters, x::Vector{Float64}, δ::Float64, subproblem_solver_method::String=subproblem_solver_methods.OPTIMIZATION_METHOD_DEFAULT)
    @assert(δ >= 0)
	#Termination conditions
	@info problem
	termination_conditions_struct = problem.termination_conditions_struct
